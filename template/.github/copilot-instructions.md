...
Five MCP servers configured in `.vscode/mcp.json` (gitignored — contains credentials):

| Server | Transport | Purpose |
|--------|-----------|---------|
| **GitHub** | HTTP | Repository operations, issues, PRs via Copilot |
| **Playwright** | Stdio (npx) | Browser automation, navigation, screenshots |
| **Redis** | Stdio (uvx) | Cache operations: GET, SET, DEL, HGETALL, key scan |
| **MySQL** | Stdio (npx) | Database queries: SELECT, INSERT, UPDATE, DELETE, DDL |
| **Git** | Stdio (uvx) | Git log, diff, blame, branch operations |

**Key constraint:** VS Code does NOT expand shell env vars in `mcp.json` — uses literal values only.
- `mcp.json` is auto-generated by `setup.sh` with literal credentials from `.env`
- If you change database/Redis passwords, update both `.env` AND `.vscode/mcp.json`
- See `docs/MCP-SETUP.md` for customization and adding new servers
- The `sql-guard.sh` hook blocks destructive SQL via MCP MySQL
